...
public class InventoryResultQueueService {
    ...
    private String resolveQueueUrl() {
        try {
            return sqsClient.getQueueUrl(GetQueueUrlRequest.builder()
                    .queueName(properties.getInventoryResultQueueName())
                    .build()).queueUrl();
        } catch (QueueDoesNotExistException ex) {
            if (!properties.isAutoCreateQueue()) {
                throw ex;
            }
            log.info("[SQS] creating queue={}", properties.getInventoryResultQueueName());
